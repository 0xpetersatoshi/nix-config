...
#
# Ported from Omarchy's `omarchy-capture-region`. Prints the picked geometry in
# slurp's "X,Y WxH" format, or exits 1 when the pick is cancelled. Shared by the
# screenshot and OCR commands so the picker UX stays identical.
#
...
#                   PID as the first output line; the caller owns killing it
...
# jq program: the $-vars below are jq's, not the shell's.
...
# RESOLVED_RECT; returns 1 when no candidate contains the point.
resolve_rect_at() {
  local x=$1 y=$2
  local rect area
...
# back to it, and leave that point in WARP_X / WARP_Y.
warp_point_in() {
...
# --- keyboard control modes (invoked by the transient binds below) -----------
...
if [[ ${1:-} == "--select-window" ]]; then
  ...
  *)
  ...
  if [[ -n $target && -n ${warp_points[$target]:-} ]]; then
    ...
    hyprctl dispatch movecursor "$target_x" "$target_y" >/dev/null
  fi
  exit 0
fi
